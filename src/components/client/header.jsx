   ...
   return (
      <header className="fixed shadow-lg w-full h-[65px] z-10 bg-slate-100 text-slate-500 font-semibold flex justify-between md:flex md:justify-between md:items-center">
         <Link to="" className="text-xl">
            <h1 className="font-bold text-2xl text-yellow-300 text-shadow-md mb-2 md:text-3xl ms-2">Doc <span className="text-blue-300">Portal</span></h1>
         </Link>
            ...
            isMenuOpen && (
               <nav className="fixed top-0 left-0 w-screen h-screen z-10 bg-linear-to-b from-green-100 to-white flex flex-col justify-start items-center gap-3 md:hidden">
                  ...
                  <button
                     ...
                     className="mt-20 shadow w-[90%] p-2 text-center bg-blue-300 text-slate-600 font-semibold rounded-md hover:bg-slate-500 hover:text-slate-200">Login</button>
               </nav>
            )
         }
         ...
         <div className="hidden md:block">
            <button
               onClick={() => navigate("/login")}
               className="hidden md:block md:w-[100px] bg-slate-300 text-blue-600 font-semibold px-4 py-2 rounded hover:bg-slate-500 hover:text-slate-200 me-5">
               Login
            </button>
         </div>
